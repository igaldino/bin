...
SYNCOM="rsync --delete --delete-excluded --include=.certponto --include=.cisco --include=.code42 --include=.CrashPlan --include=.crashplan --include=.git --include=.gitconfig --include=.gitignore --include=.ibm --include=.minecraft --include=.minetest --include=.config --include=.config/1Password --include=.config/Genymobile --include=.config/openttd --include=.config/VirtualBox --include=.config/gnome-boxes --include=.local --include=.local/share --include=.local/share/applications --include=.local/share/gnome-boxes --include=.local/share/openttd --include=.config/lutris --include=.local/share/lutris --include=.local/share/Steam --include=.openttd --include=.steam --include=.steampath --include=.steampid --include=.var --include=.var/app --include=.var/app/info.febvre.Komikku --include=.var/app/com.mojang.Minecraft --include=.var/app/org.openttd.OpenTTD --include=.var/app/org.tlauncher.TLauncher --include=.Genymobile --exclude=.* --exclude=.config/* --exclude=.local/* --exclude=.local/share/* --exclude=.var/* --exclude=.var/app/* --exclude=lost+found --exclude=ISOs -av"
SYNFLL="rsync --delete --delete-excluded --exclude=.cache --exclude=.local/share/Trash --exclude=.mozilla --exclude=.var/app/us.zoom.Zoom -av"
LSTFLG=-n
NETFLG=-z
DIRLST=/home/${MYUSER}/
HDDNET=hdd
